  ...
  return (
    <section id="top" className="px-3 pb-12 pt-5 sm:px-6 sm:pb-20">
      <div className="hero-stage relative mx-auto min-h-[680px] max-w-[1360px] overflow-hidden rounded-[32px] bg-white shadow-soft lg:min-h-[770px]">
        ...
        <div className="hero-product-plane absolute inset-x-[2.2%] bottom-7 top-[45%] z-[2] rounded-[26px] bg-[#efdbc7]" />

        <h1 className="hero-title absolute left-1/2 top-[80px] z-[1] w-[95%] -translate-x-1/2 text-center font-display text-[clamp(4.15rem,10vw,10.8rem)] leading-[.8] tracking-[-.07em] text-ink">
          <span className="text-tomato">НОВАЯ</span> ПИЦЦА<br />КОЛЛЕКЦИЯ
        </h1>

        <div className="hero-pizza absolute left-1/2 top-[52%] z-[3] w-[min(94vw,760px)] -translate-x-1/2 -translate-y-[35%] lg:w-[760px]">
          <motion.img
            key={pizza.id}
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={() => onAdd(pizza)}
          className="absolute bottom-[166px] right-5 z-[5] grid h-[78px] w-[78px] place-items-center rounded-full bg-tomato text-center text-[9px] font-extrabold uppercase leading-3 text-white shadow-float sm:right-9 lg:bottom-auto lg:right-[120px] lg:top-[52%] lg:h-[94px] lg:w-[94px]"
        >
          <span><Plus className="mx-auto mb-1" size={18} /><span className="block">в корзину</span></span>
        </motion.button>

        <div className="absolute right-4 top-1/2 z-[5] flex -translate-y-1/2 flex-col gap-2 sm:right-7 lg:right-[30px]">
          {pizzas.slice(0, 3).map((item) => (
            <button
              key={item.id}
              onClick={() => onSelect(item)}
              className={`h-[58px] w-[58px] overflow-hidden rounded-xl border-2 bg-[#f6d9c3] transition sm:h-[70px] sm:w-[70px] ${pizza.id === item.id ? 'border-tomato shadow-md' : 'border-white hover:border-tomato/40'}`}
